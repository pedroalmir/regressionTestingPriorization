...
public class PrevencaoConsultaUrologia extends CommandPrevencaoSaudeConsulta{

	private static final String ESPECIALIDADE_UROLOGIA = "Urologia";

	@Override
	public boolean geraCoparticipacao(GuiaConsulta guia) {

		if(!guia.getEspecialidade().getDescricao().equalsIgnoreCase(ESPECIALIDADE_UROLOGIA))
			return true;
		...
		sa.addParameter(new Equals("especialidade.descricao",ESPECIALIDADE_UROLOGIA));
		sa.addParameter(new LowerEquals("dataAtendimento", new Date()));
		
		if(idadeValida(segurado.getIdade(), 40, 1000) && segurado.getSexo() == SexoEnum.MASCULINO.value().intValue()){
