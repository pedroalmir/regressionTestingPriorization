...
public class StrategyRemessaBancoCef implements StrategyRemessaBancoInterface<Conta, InformacaoFinanceiraInterface, ArquivoDeEnvio>{
	...
	public void executar(Conta conta, InformacaoFinanceiraInterface informacaoFinanceira) throws Exception {
		...
		//TODO Mudar o nome de identificacaoDoCliente para um nome mais sugestivo
		String identificacaoDoCliente = String.valueOf(informacaoFinanceira.getMatricula());
		
		identificacaoDoCliente = StringUtils.leftPad(identificacaoDoCliente, 6, "0");
		
		String agencia = informacaoFinanceira.getAgencia().substring(0,4); 
		String contaCorrente = Utils.removerStringEsquerda(informacaoFinanceira.getConta(), "0"); 
		...
		if(StrategyRemessaBancoInterface.CAIXA_ECONOMICA == Integer.parseInt(banco.getCodigoFebraban())){ // CAIXA ECONOMICA
			codigoOperacao = informacaoFinanceira.getOperacao().trim().substring(0, 3);
			contaCorrente = StringUtils.leftPad(contaCorrente, 9, "0");
			contaCorrente = StringUtils.rightPad(contaCorrente, 11, " ");
		}
		...
		String agendamentoCliente = "000000";
